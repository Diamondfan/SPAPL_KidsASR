...
from transformers.utils import TensorType, logging, PaddingStrategy 
from data.audio_utils import mel_filter_bank
...
class WhisperFeatureExtractor(SequenceFeatureExtractor):
    ...
    model_input_names = ["input_features", "perturbed_features"]
    ...
    def _np_extract_fbank_features(self, waveform: np.array, vtlp_rate: float = 1.0, use_pif: bool = False) -> np.ndarray:
        """
        ...
        if use_pif:
            vtlp_rate = np.random.choice([0.9, 1.1])
    ...
    def __call__(
        ...
        **kwargs,
        ...
        if is_batched:
            if sp_rate == 1.0 and pitch_level == 0.0:
                raw_speech = [np.asarray([speech], dtype=np.float32).T for speech in raw_speech]
            elif sp_rate != 1.0 and pitch_level == 0.0:
                raw_speech = [np.asarray([librosa.effects.time_stretch(np.asarray(speech, dtype=np.float32), rate=sp_rate)]).T for speech in raw_speech]
            elif sp_rate == 1.0 and pitch_level != 0.0:
                raw_speech = [np.asarray([librosa.effects.pitch_shift(np.asarray(speech, dtype=np.float32), sr=sampling_rate, n_steps=pitch_level)]).T for speech in raw_speech]
            else:
            ...
            if sp_rate != 1.0 and pitch_level == 0.0:
                raw_speech = librosa.effects.time_stretch(raw_speech, rate=sp_rate)
            elif sp_rate == 1.0 and pitch_level != 0.0:
                raw_speech = librosa.effects.pitch_shift(raw_speech, sr=sampling_rate, n_steps=pitch_level)
        ...
        input_features_ = padded_inputs.get("input_features").transpose(2, 0, 1)

        input_features = [self._np_extract_fbank_features(waveform, vtlp_rate) for waveform in input_features_[0]]
        
        if use_pif:
            perturbed_features = [self._np_extract_fbank_features(waveform, vtlp_rate, use_pif) for waveform in input_features_[0]]
        
        if isinstance(input_features[0], List):
            padded_inputs["input_features"] = [np.asarray(feature, dtype=np.float32) for feature in input_features]
            if use_pif:
                padded_inputs["perturbed_features"] = [np.asarray(feature, dtype=np.float32) for feature in perturbed_features]
        else:
            padded_inputs["input_features"] = input_features
            if use_pif:
                padded_inputs["perturbed_features"] = perturbed_features
        ...
        **kwargs,
        ...
        if is_batched:
            if sp_rate == 1.0 and pitch_level == 0.0:
                ...
                raw_speech = [librosa.effects.time_stretch(np.asarray(speech, dtype=np.float32), rate=sp_rate) for speech in raw_speech]
            elif sp_rate == 1.0 and pitch_level != 0.0:
                raw_speech = [librosa.effects.pitch_shift(np.asarray(speech, dtype=np.float32), sr=sampling_rate, n_steps=pitch_level) for speech in raw_speech]
            else:
                raise NotImplementedError
